...
pub tok.tok_t **compile(tok.pool_t *p, tok.tok_t **in, size_t n) {
...
int compile_if(tok.pool_t *p, tok.tok_t *x, tok.tok_t *body[], int added) {
	// Tests the condition and skips the ok branch if false.
	tok.tok_t *tst = tok.newlist(p);
	tst->items[tst->nitems++] = tok.newsym(p, "__test_and_jump_if_false");
	tst->items[tst->nitems++] = x->items[1];
	body[added++] = tst;

	// The ok branch with an end marker.
	body[added++] = x->items[2];
	body[added++] = tok.newsym(p, "__end");
...
int compile_cond(tok.pool_t *p, tok.tok_t *cond, tok.tok_t *body[], int added) {
